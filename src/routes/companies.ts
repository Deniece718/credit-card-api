...
router.get('/:companyId/cards/allData', async(req: Request, res: Response) => {
    ...
    await Promise.all(cards.map(async (card) => {
        const transactions = (await Transaction.find({ cardId: card.id })).map((transaction) => ({
            id: transaction._id,
        ...
        cardsData.push({
            ...
            remainingSpend: {
                used: currentMonthTransactions.reduce((sum, txn) => sum - Math.abs(txn.amount), card.creditLimit),
